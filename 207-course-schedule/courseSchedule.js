...
const canFinish = (numCourses, prerequisites) => {
  ...
  const graph = buildAdjGraph(numCourses, prerequisites);
...
const buildAdjGraph = (numCourses, prerequisites) => {
  const graph = new Map();
  ...
  for (let prerequisite of prerequisites) {
    const [courseA, courseB] = prerequisite;
    graph.get(courseA).push(courseB);
  }
